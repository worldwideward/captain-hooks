...
function check_and_upgrade_talisman_binary() {
  if [[ -n "${TALISMAN_HOME:-}" && "$TALISMAN_SKIP_UPGRADE" != "true" ]]; then
    ...
    CURRENT_VERSION=$(/home/w/.talisman/bin/talisman_linux_amd64 --version | grep -Eo '[0-9]+\.[0-9]+\.[0-9]+')
...
CMD="$HOME/.talisman/bin/talisman_linux_amd64 ${DEBUG_OPTS} --githook ${HOOKNAME} ${INTERACTIVE}"
echo_debug "ARGS are $@"
